# ─────────────────────────────────────────────────────────────
# Complete rebuild: 10-page Mausoleum Walk White & Gold Marble
# Asset mapping from disk scan:
...
STARS = "stars.png"
MES = [f"me ({i}).jpg" for i in range(1, 11)]  # me (1).jpg ... me (10).jpg
LETTER = "letter.jfif"
GIFTS = ["gifts (1).jpg", "gifts (2).jpg"]
OUR_GIFT = "moment 4.jpg"  # user said moment(4).jfif -> actual is moment 4.jpg
BG_DECOS = [
...
]  # note: bg (4).jfif missing, skip it
...
/* Vignette corners for mausoleum feel */
.vignette {{
    position: fixed;
    inset: 0;
    pointer-events: none;
...
/* Page base: like a marble slab in a mausoleum corridor */
.page {{
    position: absolute;
    inset: 0;
    display: flex;
...
/* MAUSOLEUM TRANSITIONS: walk forward through corridor */
.page.active {{
    opacity: 1;
    pointer-events: all;
...
/* Walk out animation: current page recedes INTO the distance (past) */
.page.leaving-back {{
    opacity: 0;
    pointer-events: none;
...
/* ─── NAV BUTTONS (marble, walk-through) ─── */
.nav-btn {{
    position: fixed;
    bottom: 22px;
...
/* Progress dots (mausoleum chambers) */
#page-dots {{
    position: fixed;
    left: 50%;
    bottom: 30px;
...
.mf.lift:active, .mf.lift:hover {{ transform: translateY(-2px) scale(1.01); }}
.mf.lift {{ transition: transform 0.3s ease; }}

/* tiny scattered background marble frames */
.smf {{
    position: absolute;
    z-index: 2;
    opacity: 0.72;
...
/* arch frame: arched top marble slab */
.arch-shape {{
    position: absolute;
    inset: 0;
    background: linear-gradient(160deg, var(--marble-white), var(--marble-cloud));
...
/* Vertical timeline line (underneath) */
.tl-spine {{
    position: absolute;
    left: 50%;
    top: 40px;
...
/* stars overlay (constellations night we met) */
.ast-stars {{
    position: absolute;
    inset: -6%;
...
.ast-moon .mf {{ padding: 10px; }}
.ast-moon .mf img {{
    aspect-ratio: 1 / 1;
    object-fit: cover;
    border-radius: 50% !important;
}}
.ast-moon .mf {{ border-radius: 50%; }}
...
/* ════════════════════════════════════════════════
   PAGE 5 – "Some photos of me" me 1..10
   ════════════════════════════════════════════════ */
.me-grid {{
    width: 100%;
    max-width: 460px;
...
/* me 1 is index 0, me 6 is index 5 -> span 2 */

/* ════════════════════════════════════════════════
   PAGE 6 – DEAR AYA (letter inside letter.jfif)
   ════════════════════════════════════════════════ */
.letter-wrap {{
    position: relative;
    width: 100%;
...
/* ════════════════════════════════════════════════
   PAGE 8 – 12 things I love about you (12 months)
   ════════════════════════════════════════════════ */
.twelves {{
    width: 100%;
    max-width: 460px;
...
/* ════════════════════════════════════════════════
   PAGE 9 – OUR GIFTS (gifts 1, 2 top row + moment4 bottom)
   ════════════════════════════════════════════════ */
.gifts-wrap {{
    width: 100%;
    max-width: 460px;
...
/* Album close / fade out on ending transition (triggered in JS via .final class on body) */
body.fade-out {{
    transition: opacity 4s ease-out;
    opacity: 0;
}}

/* ── Flower pop-up note (press anywhere if flower is kept / optional) ── */
.flower-note {{
    position: fixed;
    top: 50%;
    left: 50%;
...
<div class="page active" id="p0">
    <div class="portal-wrap" id="portal-wrap">
        ...
        <div class="portal-sub">a mausoleum of memories · 10 chambers</div>
    ...
    <div class="center-x">
        ...
        <div class="tl-wrap">
            ...
            <div class="tl-moments">
                <div class="tl-row">
                    <div class="tl-photo"><div class="mf lift"><img loading="lazy" src="{MOMENTS[0]}" alt="moment one"><div class="cap">I</div></div></div>
                    <div class="tl-card">
                ...
                <div class="tl-row">
                    <div class="tl-photo"><div class="mf lift"><img loading="lazy" src="{MOMENTS[1]}" alt="moment two"><div class="cap">II</div></div></div>
                    <div class="tl-card">
                ...
                <div class="tl-row">
                    <div class="tl-photo"><div class="mf lift"><img loading="lazy" src="{MOMENTS[2]}" alt="moment three"><div class="cap">III</div></div></div>
                    <div class="tl-card">
                ...
                <div class="tl-row">
                    <div class="tl-photo"><div class="mf lift"><img loading="lazy" src="{MOMENTS[3]}" alt="moment four"><div class="cap">IV</div></div></div>
                    <div class="tl-card">
    ...
    <div class="center-x">
        ...
        <div class="ast-wrap">
            <div class="ast-stage">
                ...
                <div class="ast-center"><div class="mf sway" style="--rot:0deg;"><img loading="lazy" src="{MEET_NIGHT}" alt="the night we met"></div></div>
...
<div class="page" id="p4">
    <div class="center-x">
        ...
        <div class="me-grid" id="me-grid">
            <!-- me (1).jpg landscape -> span2 (index 0) -->
            <!-- me (6).jpg landscape -> span2 (index 5) -->
            <!-- order: 1(span2), 2, 3, 4, 5, 6(span2), 7, 8, 9, 10 -->
            <!-- 10 photos total: span2 + 4 singles + span2 + 4 singles = 2+4+2+4=12 columns, but we need 10 items over grid of 4 cols.
                 Arrange row by row:
                 Row 1: [me1 span2] [me2] [me3]
                 Row 2: [me4] [me5] [me6 span2]
                 Row 3: [me7] [me8] [me9] [me10]
            -->
            <div class="me span2"><div class="mf lift"><img loading="lazy" src="{MES[0]}" alt="me 1"></div></div>
            <div class="me"><div class="mf lift"><img loading="lazy" src="{MES[1]}" alt="me 2"></div></div>
            <div class="me"><div class="mf lift"><img loading="lazy" src="{MES[2]}" alt="me 3"></div></div>
            <div class="me"><div class="mf lift"><img loading="lazy" src="{MES[3]}" alt="me 4"></div></div>
            <div class="me"><div class="mf lift"><img loading="lazy" src="{MES[4]}" alt="me 5"></div></div>
            <div class="me span2"><div class="mf lift"><img loading="lazy" src="{MES[5]}" alt="me 6"></div></div>
            <div class="me"><div class="mf lift"><img loading="lazy" src="{MES[6]}" alt="me 7"></div></div>
            <div class="me"><div class="mf lift"><img loading="lazy" src="{MES[7]}" alt="me 8"></div></div>
            <div class="me"><div class="mf lift"><img loading="lazy" src="{MES[8]}" alt="me 9"></div></div>
            <div class="me"><div class="mf lift"><img loading="lazy" src="{MES[9]}" alt="me 10"></div></div>
        </div>
...
<div class="page" id="p8">
    <div class="center-x">
        ...
        <div class="gifts-wrap">
            <div class="gifts-top">
                <div class="gift-card">
                    <div class="mf lift"><img loading="lazy" src="{GIFTS[0]}" alt="gifts from her to me"></div>
                    <div class="gift-title">from you → to me</div>
                </div>
                <div class="gift-card">
                    <div class="mf lift"><img loading="lazy" src="{GIFTS[1]}" alt="gifts from me to her"></div>
...
// ─── SCATTERED BG.JFIF 1..28 in marble frames across pages ───
const BGS = {JSON.stringify(BG_DECOS)};
function scatterDeco() {{
    pages.forEach((p, idx) => {{
        if (idx === 0 || idx === 9) return; // keep portal and ending clean of clutter
        const count = 6;
        for (let i = 0; i < count; i++) {{
            ...
            const w = 40 + Math.random() * 36; // 40-76 px width
            d.style.width = w + 'px';
...
// ─── MAUSOLEUM WALK-THROUGH: go to page n ───
function goTo(n) {{
    ...
    // Mausoleum walk: moving forward -> old page recedes (translateZ negative), new page arrives from distance (translateZ negative -> 0)
    // Moving back -> old page advances & blasts off (translateZ positive), new page arrives from front (translateZ positive -> 0)
    ...
    // Start fade/translate on new after small frame so it's perceivable
...
function onPageChanged(n) {{
    ...
    // home shows once we are past the portal
    homeBtn.classList.toggle('show', n !== 0);

    // Ending: trigger fade-out after 1.2s, also music fade
    if (n === total - 1) {{
        setTimeout(() => {{
...
// Start music on first user interaction anywhere on page
const firstInteract = () => {{
    startMusic();
...
// ─── PORTAL OPEN on enter button ───
const enterBtn = document.getElementById('enter-btn');
const portalWrap = document.getElementById('portal-wrap');
enterBtn.addEventListener('click', () => {{
    startMusic();
    if (portalWrap) portalWrap.classList.add('portal-open');
    // trigger the walk-through AFTER portal burst
    setTimeout(() => {{
        goTo(1);
    }}, 900);
}});

// ─── Optional click on marble frames -> tiny lift (handled by .mf.lift :active)
// Optional flower note hidden handler (click background empty space -> tiny note)
...
// Click on the constellation stars for easter egg
...
print("Rebuilt index.html ✓  (10 pages · Mausoleum walk-through · White & Gold Marble)")
print(f"Total BG deco assets available: {len(BG_DECOS)}")
print(f"Music file: {MUSIC}")
